...
Ray Camera::generate_ray(double x, double y) const {
  ...
  // to go from image coordinates to camera space, you subtract 0.5 from both x and y
  // this is to align the centers of both planes (1,1) -> (0.5, 0.5)
  // then you scale x by 2 * tan(hFov/2) and y by 2 * tan(vFov/2) so
  // the top right corners align. (0.5, 0.5) -> (tan(hFov/2), tan(vFov/2))
    
  x -= 0.5;
  y -= 0.5;
  x = x * 2.0 * tan(0.5 * radians(hFov));
  y = y * 2.0 * tan(0.5 * radians(vFov));
